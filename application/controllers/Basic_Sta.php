...
class Basic_Sta extends CI_Controller {
    private $per_page=8;
    private $pagenation=array('8',"10","20","50","100","all");
    private $admin_url="statistics";
    private $table="basic_stastics";
    private $data=array();
    ...
    public function add(){
        ...
        $data=array(
            "id"=>$this->removeHtmlTags($this->url($this->input->post("id")).date()),
    ...
    public function update(){
        ...
        $data=array(
            ...
            "title"=>$this->removeHtmlTags($this->input->post("title")),
            "happy_customer"=>nl2br($this->input->post("happy_customer")),
            "successful_deals"=>nl2br($this->input->post("successful_deals")),
            "awards"=>nl2br($this->input->post("awards")),
            "picture"=>$this->removeHtmlTags($this->input->post("current_picture"))
        );

        /*check if picture is empty*/
        if ($_FILES['picture']['error']!=4) {
            $file_name=$this->do_upload("./images","picture")['upload_data']['file_name'];
            $data['picture']=$file_name;
        }
    ...
    public function load_page_content($ref){
        ...
        $this->load->view('dashboard/dashboard_sta');
